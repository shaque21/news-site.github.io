                    ...
                    <?php
                      ...
                      if(mysqli_num_rows($result) > 0){
                        ...
                        <?php
                          ...
                          echo "<h2 class ='alert alert-danger'>No Records Found.</h2>";
                        }
                          //pagination start
                        $query_of_search_pag = "SELECT * FROM post LEFT JOIN user ON post.author = user.user_id
                        WHERE post.title LIKE '%{$search_term}%' 
                        OR post.description LIKE'%{$search_term}%' OR user.username LIKE '%{$search_term}%'";

                    $result_of_search_pag = mysqli_query($connect, $query_of_search_pag) or die("Query failed : search");
